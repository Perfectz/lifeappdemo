...
- **Context harness:** [`src/domain/aiContext.ts`](../src/domain/aiContext.ts) builds an `AIAppContext` (active goals, open tasks, today's plan, recent metrics, journal entries, notes, latest report, durable memories, the user-reported training profile/constraints, and **derived behavioral insight highlights** from `insights.ts`) and formats it for the prompt. Scheduled recovery days and per-day training targets are preserved instead of reverting to a three-session default. The chat UI provides a compact context inspector with source limits and links. Multi-turn: the last ~10 history turns are included.
- **Identity and health grounding:** the coach is told the user is becoming a specific future self (their About Me profile, when present), and to frame guidance around "what would that future self do?" Logged health readings are explicitly observations, not diagnoses; saved medication, condition, and injury memories are treated as user-reported safety constraints, with bounded language and professional referral where appropriate.
- **Gmail grounding:** when a connected user explicitly asks about email, inbox, or Gmail, the server fetches a bounded digest of recent unread messages and appends sender, subject, and snippet data to that request. Gmail is not fetched for unrelated coaching. The Agent may propose `create_email_draft`; confirmation creates a Gmail draft, never a sent message.
