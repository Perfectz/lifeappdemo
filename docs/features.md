...
The day's command center. Shows a deterministic **daily brief** that lists exactly what still needs the user today — vitals, the three workouts ("N/3 done"), and whether today is planned — each as a focus item with a one-tap CTA and deep link, with overdue items floated to the top. When an OpenAI key is present, a short coach-written briefing sentence is layered on top of the deterministic facts.
- Route: `/dashboard`
...
### Daily Fitness (3 sessions)
Tracks the required three daily training sessions and lets the user log each: one **strength** session from a five-day split (each exercise has Free Weight / Machine / Kettlebell variants with form instructions and a form-video search link), one **cardio** session (walk, run, jog, DDR, exercise bike + weight vest), and one **martial arts** session (Bas Rutten boxing beginner/advanced, Shidokan kickboxing/karate).
- Route: `/fitness`
...
### Quest Log / Tasks
Capture and clear "quests" (tasks). Tasks have a priority, tags (health/work/content/social/admin/learning), optional due/planned dates, and a status (todo/done/archived). Includes quick-add and grouping.
- Route: `/tasks`
...
A curated, human-authored "About Me" knowledge base with sections (Profile, Health, Nutrition, Training, Goals, Coaching Preferences, People, Constraints, Other). This is the identity context the AI coach reads. Authored and stored by the user; never committed to source.
- Route: `/profile`
...
### AI Coach chat (multi-turn, photo + voice)
A read-only conversational coach. Multi-turn (recent history is included), grounded in the user's recent app data and About Me profile. The coach can propose data/task changes (create/complete/defer/archive task, log metric, create journal entry, propose a daily plan, generate a report) that the user must confirm before they apply. The panel also integrates photo capture and voice.
- Route: `/coach`
...
- Surfaced in the AI Coach panel / voice components.
